...
- **story**: 반드시 **한국어**로 작성하세요. 인물/사물 ID를 사용하여 상황을 설명하세요. **story는 단일 패널이 아니라 여러 연속된 패널별로 (비주얼적으로 유사하거나 같은 장면에 카메라뷰만 다른 경우, 또는 한 장면을 반복적으로 주시하는 경우 등) 큰 묶음으로 표출되어야 합니다.** 같은 묶음에 속하는 클립(clip)들은 'Story 1: [내용]', 'Story 2: [내용]'과 같은 형식으로 'story' 필드 내용이 **동일하게 유지**(반복 기재)되어야 합니다.
