...
from app.services.firestore import get_job_queue_service, get_image_frame_service
from app.services.s3 import download_image, generate_image_filename, upload_image
...
async def check_cancellation(job_id: str) -> None:
    ...
    if job and job.cancellation_requested:
        logger.info(f"Cancellation detected for image job {job_id}")
...
async def process_image_generation(
    ...
    logger.info(f"[{worker_id}] ========== Starting image generation for job {job_id} ==========")

    # Fetch job from Firestore
    job = await job_queue_service.get_job(job_id)
    if not job:
        logger.error(f"[IMAGE-GEN] Job {job_id} not found in Firestore")
        return {"job_id": job_id, "status": "error", "error": "Job not found"}

    logger.info(f"[IMAGE-GEN] Job {job_id} loaded: type={job.type}, project={job.project_id}, frame={job.frame_id}, status={job.status}")
    logger.info(f"[IMAGE-GEN] Prompt: {job.prompt[:100]}...")
    logger.info(f"[IMAGE-GEN] Reference URLs: {len(job.reference_urls or [])} images")
    logger.info(f"[IMAGE-GEN] Aspect ratio: {job.aspect_ratio}")
    ...
    try:
        ...
        logger.info(f"[IMAGE-GEN] {job_id} ========== JOB COMPLETED SUCCESSFULLY ==========")
        logger.info(f"[IMAGE-GEN] {job_id} - Image URL: {result['image_url']}")
        logger.info(f"[IMAGE-GEN] {job_id} - S3 File: {result['s3_file_name']}")
        return {
            "job_id": job_id,
            "status": "completed",
        ...
        logger.warning(f"[IMAGE-GEN] {job_id} - Job was cancelled")
        await _handle_cancellation(job_id, state_machine)
        return {"job_id": job_id, "status": "cancelled"}

    except VideoJobError as e:
        logger.error(f"[IMAGE-GEN] {job_id} - VideoJobError: {e.code.value} - {e.message}")
        return await _handle_error(job, e, state_machine, custom_api_key)

    except Exception as e:
        logger.exception(f"[IMAGE-GEN] {job_id} - Unexpected error: {type(e).__name__}: {str(e)}")
...
async def _stage_prepare(
    ...
    logger.info(f"[IMAGE-GEN] {job.id} - Stage 1: PREPARING (fetching reference images)")
    ...
    logger.debug(f"[IMAGE-GEN] {job.id} - Status updated to PREPARING in Firestore")

    reference_images: list[bytes] = []

    if job.reference_urls:
        logger.info(f"[IMAGE-GEN] {job.id} - Fetching {len(job.reference_urls)} reference images from S3")
        for i, url in enumerate(job.reference_urls):
            logger.debug(f"[IMAGE-GEN] {job.id} - Fetching reference {i+1}/{len(job.reference_urls)}: {url}")
            try:
                img_bytes = await download_image(url)
                reference_images.append(img_bytes)
                logger.info(f"[IMAGE-GEN] {job.id} - ✓ Fetched reference {i+1}: {len(img_bytes)} bytes")
            except Exception as e:
                logger.warning(f"[IMAGE-GEN] {job.id} - ✗ Failed to fetch reference image {url}: {e}")
                # Continue without this reference
    else:
        logger.info(f"[IMAGE-GEN] {job.id} - No reference images to fetch")

    await job_queue_service.update_job(job.id, progress=0.2)
    logger.info(f"[IMAGE-GEN] {job.id} - Stage 1 complete: {len(reference_images)} references loaded")
...
async def _stage_generate(
    ...
    logger.info(f"[IMAGE-GEN] {job.id} - Stage 2: GENERATING image with Gemini")
    ...
    logger.debug(f"[IMAGE-GEN] {job.id} - Status updated to GENERATING in Firestore")
    ...
    logger.info(f"[IMAGE-GEN] {job.id} - Request built: prompt_len={len(job.prompt)}, style_prompt={bool(job.style_prompt)}, refs={len(reference_images)}, ratio={job.aspect_ratio}")

    # Validate
    validation_error = gemini_image_provider.validate_request(request)
    if validation_error:
        logger.error(f"[IMAGE-GEN] {job.id} - Validation failed: {validation_error}")
        raise VideoJobError.invalid_request(validation_error)
    logger.debug(f"[IMAGE-GEN] {job.id} - Request validation passed")

    # Generate image
    logger.info(f"[IMAGE-GEN] {job.id} - Calling Gemini API...")
    try:
        image_bytes = await gemini_image_provider.generate_image(request, api_key)
        logger.info(f"[IMAGE-GEN] {job.id} - ✓ Image generated successfully: {len(image_bytes)} bytes")
    except Exception as e:
        logger.error(f"[IMAGE-GEN] {job.id} - ✗ Gemini API error: {type(e).__name__}: {str(e)}")
...
async def _stage_upload(
    ...
    logger.info(f"[IMAGE-GEN] {job.id} - Stage 3: UPLOADING to S3")
    ...
    logger.debug(f"[IMAGE-GEN] {job.id} - Status updated to UPLOADING in Firestore")

    # Generate S3 filename
    s3_file_name = generate_image_filename(job.project_id, job.frame_id or job.id, job.id)
    logger.debug(f"[IMAGE-GEN] {job.id} - Generated S3 filename: {s3_file_name}")

    # Upload to S3
    logger.info(f"[IMAGE-GEN] {job.id} - Uploading {len(image_bytes)} bytes to S3...")
    try:
        image_url = await upload_image(image_bytes, s3_file_name, "image/png")
        logger.info(f"[IMAGE-GEN] {job.id} - ✓ Uploaded to S3: {s3_file_name}")
        logger.debug(f"[IMAGE-GEN] {job.id} - Image URL: {image_url}")
    except Exception as e:
        logger.error(f"[IMAGE-GEN] {job.id} - ✗ S3 upload failed: {e}")
...
async def _stage_complete(
    ...
    job_queue_service = get_job_queue_service()
    image_frame_service = get_image_frame_service()

    logger.info(f"[IMAGE-GEN] {job.id} - Stage 4: COMPLETING")

    state_machine.transition_to(JobStatus.COMPLETED)

    # Update job queue
    logger.debug(f"[IMAGE-GEN] {job.id} - Updating job_queue status to COMPLETED")
    ...
    logger.info(f"[IMAGE-GEN] {job.id} - ✓ Job queue updated")

    # Update image frame in project Firestore
    if job.frame_id:
        logger.debug(f"[IMAGE-GEN] {job.id} - Updating image frame in project/{job.project_id}/imageGen/frames/{job.frame_id}")
        await image_frame_service.update_frame_status(
        ...
        logger.info(f"[IMAGE-GEN] {job.id} - ✓ Image frame updated")
    else:
        logger.warning(f"[IMAGE-GEN] {job.id} - No frame_id, skipping frame update")
...
async def _handle_error(
    ...
    from app.workers.tasks import retry_failed_image_job
    ...
    if error.retryable and retry_state.can_retry():
        ...
        # Update image frame status to show error
        if job.frame_id:
            await image_frame_service.update_frame_status(
                project_id=job.project_id,
                frame_id=job.frame_id,
        ...
        await retry_failed_image_job.kiq(job.id, delay, custom_api_key)
