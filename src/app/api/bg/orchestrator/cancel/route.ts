...
export async function POST(request: NextRequest) {
  try {
    ...
    const body: CancelBgJobRequest = await request.json();
    ...
    const cancelUrl = buildInternalServiceUrl(
      ORCHESTRATOR_URL,
      `/api/v1/bg-jobs/${encodeURIComponent(safeJobId)}/cancel`
    );
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.detail || "Failed to cancel bg job" },
    ...
    console.error("Error cancelling bg job:", error);
