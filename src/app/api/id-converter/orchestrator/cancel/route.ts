...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[IdConverterOrchestrator] Cancelling job:", body.jobId);

    const response = await fetch(`${ORCHESTRATOR_URL}/api/v1/id-converter-jobs/${body.jobId}/cancel`, {
      method: "POST",
      headers: {
    ...
    console.log("[IdConverterOrchestrator] Cancel response status:", response.status, "body:", responseText.substring(0, 500));
    ...
    console.error("Error cancelling ID converter job:", error);
