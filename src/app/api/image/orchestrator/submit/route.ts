...
interface OrchestratorImageSubmitRequest {
  projectId: string;
  frameId: string;
  sceneIndex: number;
  clipIndex: number;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.detail || "Failed to submit image job" },
    ...
    console.error("Error submitting to image orchestrator:", error);
