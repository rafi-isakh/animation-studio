...
interface KreaStyleConverterJobSubmitRequest {
  projectId: string;
  sessionId: string;
  panelId: string;
  fileName: string;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body: KreaStyleConverterJobSubmitRequest = await request.json();
    ...
    const response = await fetch(`${ORCHESTRATOR_URL}/api/v1/krea-style-converter-jobs/submit`, {
      method: "POST",
      headers: {
