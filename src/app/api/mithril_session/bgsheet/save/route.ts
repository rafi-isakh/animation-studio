...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1. Delete existing images in bgsheet_images/ folder (to replace old session)
    const imagesPrefix = `${MITHRIL_S3_FOLDER}/bgsheet_images/`;
    ...
    const bgImages: BgImage[] = sessionData.bgImages || [];
    const uploadedImages = await Promise.all(
      bgImages.map(async (img: BgImage) => {
        if (!img.base64) {
          // No image data, just return metadata with s3Key placeholder
          return { ...img, base64: undefined, s3Key: "" };
        }

        const imageKey = `${MITHRIL_S3_FOLDER}/bgsheet_images/${img.id}.jpg`;
        ...
        // Return metadata with s3Key instead of base64
        return {
          id: img.id,
          type: img.type,
