...
interface ModelsLabStyleConverterJobSubmitRequest {
  projectId: string;
  sessionId: string;
  panelId: string;
  fileName: string;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body: ModelsLabStyleConverterJobSubmitRequest = await request.json();
    ...
    const response = await fetch(`${ORCHESTRATOR_URL}/api/v1/modelslab-style-converter-jobs/submit`, {
      method: "POST",
      headers: {
