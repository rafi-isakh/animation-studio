...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("[StorySplitterOrchestrator] Getting status for job:", jobId);

    const response = await fetch(`${ORCHESTRATOR_URL}/api/v1/story-splitter-jobs/${jobId}/status`, {
      method: "GET",
      headers: {
    ...
    console.log("[StorySplitterOrchestrator] Status response:", response.status, "body:", responseText.substring(0, 500));
