...
export default async function SuccessPage({ searchParams }: { searchParams: { payment_intent: string } }) {
    ...
    return (
        <div className="flex justify-center items-center mx-auto">
            <div className={styles.paymentStatus} id="payment-status">
                <div className={styles.statusIcon} id="status-icon" style={{ backgroundColor: STATUS_CONTENT_MAP[status as keyof typeof STATUS_CONTENT_MAP].iconColor }}>
                ...
                {paymentIntent && <div className={styles.detailsTable} id="details-table">
                    <table className={styles.stripeTable}>
                        <tbody>
                            <tr>
                                <td className={styles.TableLabel}>id</td>
                                <td className={styles.TableContent} id="intent-id">{paymentIntentId}</td>
                            </tr>
                            <tr>
                                <td className={styles.TableLabel}>status</td>
                                <td className={styles.TableContent} id="intent-status">{status}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>}
                {paymentIntent && <a href={`https://dashboard.stripe.com/payments/${paymentIntentId}`} className={styles.viewDetails} id="view-details" target="_blank">View details
                ...
                <a className={styles.retryButton} id="retry-button" href="/stars">Back</a>
            </div>
        </div>
    );
}