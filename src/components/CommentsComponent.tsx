...
const CommentsComponent = ({ chapterId }: { chapterId: string }) => {
    ...
    const handleAddComment = async (event: React.FormEvent) => {
        ...
        if (commentContent) {
            ...
            if (!user) {
                ...
                const response = await fetch('https://toonyzbackend.site/api/add_comment', {
                    method: 'POST',
                    headers: {
                ...
                if (!response.ok) {
                    ...
                    const res = await fetch(`https://toonyzbackend.site/api/get_comments?chapterId=${chapterId}`)
                        .then(data => data.json())
    ...
    useEffect(() => {
        ...
        const fetchChapter = async() => {
            const res = await fetch(`https://toonyzbackend.site/api/get_chapter_byid?id=${chapterId}`)
                .then(data => data.json());
            setChapter(res);
        }
        fetchComments();
