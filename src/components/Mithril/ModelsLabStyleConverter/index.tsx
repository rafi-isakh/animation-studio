...
import {
  subscribeToSessionModelsLabStyleConverterJobs,
...
// ── Helpers ───────────────────────────────────────────────────────────────────
const DEBUG = false;
const log = (...args: unknown[]) => DEBUG && console.log('[ModelsLabStyleConverter]', ...args);
const warn = (...args: unknown[]) => DEBUG && console.warn('[ModelsLabStyleConverter:WARN]', ...args);
...
export default function ModelsLabStyleConverter() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToSessionModelsLabStyleConverterJobs(sessionId, (jobs) => {
  ...
  const submitSinglePanel = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        const response = await fetch('/api/modelslab-style-converter/orchestrator/submit', {
          method: 'POST',
  ...
  const handleClearAll = useCallback(async () => {
    ...
    const confirmed = window.confirm('Clear the ModelsLab Style Converter workspace? This removes all local and persisted panel data for this project.');
    if (!confirmed) {
      return;
    }
  ...
  const handleRemix = useCallback(async (id: string, remixPrompt: string) => {
    const panel = panelsRef.current.find((p) => p.id === id);
    ...
    // Map AspectRatio to what the remix API accepts
    const ratioMap: Partial<Record<AspectRatio, '1:1' | '16:9' | '9:16'>> = {
      [AspectRatio.Square]: '1:1',
      [AspectRatio.Landscape]: '16:9',
      [AspectRatio.Portrait]: '9:16',
    };
    const aspectRatio = ratioMap[config.targetAspectRatio];
  ...
  const handleRemoveFileFromLibrary = useCallback((fileName: string) => {
    ...
    const panel = panelsRef.current.find(
      (p) => normalizePanelFileName(p.fileName ?? p.file?.name ?? '') === normalizePanelFileName(fileName),
    );
  ...
  const handleDownloadAll = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `modelslab-style-converter-${new Date().toISOString().slice(0, 10)}.zip`;
  ...
  const handleSaveProject = async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `modelslab-style-converter-${new Date().toISOString().slice(0, 10)}.json`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
