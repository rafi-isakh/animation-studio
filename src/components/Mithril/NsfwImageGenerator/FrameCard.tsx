    ...
    >
      ...
      <div
        className={`relative group aspect-video bg-slate-900 flex items-center justify-center rounded overflow-hidden cursor-pointer ${
          frame.isFinalized ? "ring-2 ring-green-500" : ""
        }`}
        onClick={() => {
          if (frame.isFinalized && onFinalize) {
            onFinalize(frame.id);
          } else if (frame.imageUrl) {
            onOpenModal(frame.imageUrl);
          }
        }}
      >
        ...
        {/* Finalized overlay */}
        {frame.isFinalized && frame.promptVariant && (
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute top-1 left-1 bg-green-500 text-white text-[11px] font-black px-2 py-0.5 rounded shadow-lg">
              ✓ {frame.promptVariant}
            </div>
            <div className="absolute bottom-0 left-0 right-0 bg-green-600/80 py-0.5 text-center">
              <span className="text-[8px] font-black text-white uppercase tracking-widest">
                FINALIZED · CLICK TO UNDO
              </span>
            </div>
          </div>
        )}
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-1">
            {onFinalize && frame.promptVariant && (
              <button
                onClick={() => onFinalize(frame.id)}
                className={`px-2 py-0.5 text-[10px] rounded font-black transition-colors ${
                  frame.isFinalized
                    ? "bg-green-600 text-white hover:bg-red-600"
                    : "bg-slate-700 text-slate-300 hover:bg-green-700 hover:text-white"
                }`}
                title={frame.isFinalized ? "Click to unfinalize" : `Finalize as ${frame.promptVariant}`}
              >
                {frame.isFinalized ? `✓${frame.promptVariant}` : `USE ${frame.promptVariant}`}
              </button>
            )}
            {frame.imageUrl && (
              <button
