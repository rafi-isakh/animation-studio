// Grok Aurora Image-to-Video Provider (ModelsLab grok-imagine-video-i2v)
...
const GROK_I2V_CONSTRAINTS: ProviderConstraints = {
  durations: [4, 8],
  aspectRatios: [
...
class GrokI2VProvider implements VideoProvider {
  readonly id = "grok_i2v";
  readonly name = "Grok Aurora";
  readonly description = "ModelsLab's xAI Grok image-to-video model";
  ...
  mapDuration(requestedDuration: number): number {
    const validDurations = GROK_I2V_CONSTRAINTS.durations;
  ...
  async submitJob(_request: VideoSubmitRequest): Promise<VideoSubmitResult> {
    throw new Error(
      "GrokI2VProvider.submitJob should not be called directly. Use the API route instead."
    );
  }

  async checkStatus(_jobId: string): Promise<VideoStatusResult> {
    throw new Error(
      "GrokI2VProvider.checkStatus should not be called directly. Use the API route instead."
    );
  }
}

export const grokI2VProvider = new GrokI2VProvider();
