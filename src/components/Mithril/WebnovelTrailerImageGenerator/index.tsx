...
export default function ImageGenerator() {
  ...
  // Track which frame IDs are finalized (selected by the user)
  const [finalizedImages, setFinalizedImages] = useState<Set<string>>(new Set());

  const toggleFinalizedImage = useCallback((frameId: string) => {
    setFinalizedImages((prev) => {
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 mt-3">
                  {groupFrames.map((frame, idx) => (
                    <div
                      key={frame.id}
                      className={`relative rounded-lg overflow-hidden ${finalizedImages.has(frame.id) ? 'ring-4 ring-green-500' : ''}`}
                      onClick={(e: any) => {
                        const target = e.target as HTMLElement;
                        if (target.closest('button') || target.closest('input') || target.closest('textarea') || target.closest('a')) {
                          return;
                        }
                        toggleFinalizedImage(frame.id);
                      }}
                      title={finalizedImages.has(frame.id) ? 'Click to un-finalize' : 'Click to finalize'}
                    >
                      <FrameCard
                        frame={frame}
