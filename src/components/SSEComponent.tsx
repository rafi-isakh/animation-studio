...
const SSEComponent = ({ content, chapterId }: { content: string, chapterId: string }) => {
  ...
  useEffect(() => {
    ...
    const handleTranslate = async () => {
      const response = await fetch(`https://stellandai.com:5000/api/get_translation?id=${chapterId}&language=${language}`)
  ...
  const saveTranslationToDB = async () => {
    if (text) {
      ...
      const res = await fetch('https://stellandai.com:5000/api/save_translation', {
        method: 'POST',
        headers: {
  ...
  const submitContent = async () => {
    try {
      const response = await fetch('https://stellandai.com:5000/api/send_content', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const startEventSource = (textId: string, cvid: string = '', to_continue: number = 0) => {
    const eventSource = new EventSource(`https://stellandai.com:5000/api/translate/${textId}?target=${language}&cvid=${cvid}&to_continue=${to_continue}`);
